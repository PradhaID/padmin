	...
	<?php
		...
		foreach($func->relasi_kategori_artikel($_GET['id']) as $id){
			...
			if ($no<count($func->relasi_kategori_artikel($_GET['id']))){
				echo '<a href="'.$dir->rootDir().'kategori/'.(int)$kategori['id'].'/1/'.$judul.$func->ext().'">'.$kategori['nama'].'</a>, ';
			} else echo '<a href="'.$dir->rootDir().'kategori/'.(int)$kategori['id'].'/1/'.$judul.$func->ext().'">'.$kategori['nama']."</a>.";
			$no++;
		}
	?>
