	...
	loadsection:function(rawi, bypasshooks){
  	...
  	//doload Boolean checks to see whether to load next section (true if bypasshooks param is true or onpagechangestart() event handler doesn't return false)
		...
		if (this.currentsection!=i && i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
			...
			this.paginatediv.$status.text("Soal No "+(i+1)+" dari "+this.sections.count) //update current page status text
	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			$sections.each(function(i){ //loop through $sections again
				...
				var $thestep=$('<div class="step disabledstep" />').data('section', i).html((i+1)+'<div class="smalltext" style="display: none;">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
            ...
            $stepsguide.prepend('<div class="disabledstep" style="float: left;color: #000;padding: 0px 2px;font: bold 12px Arial;">Soal No: </div>');
