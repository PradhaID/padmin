...
if (isset($halaman[$hal])){
    ...
    ?><div style="text-align: center"> Tidak ditemukan data pada halaman <?php echo $hal; ?></div><?php
}

?><div align="right"><?php
echo 'Halaman: ';
if (($hal+1)!=1){
	echo '<a href="'.$dir->rootDir().'arsip/'.($hal).'/data.ayong">&lt;&lt;</a>';
}
for ($i=1; $i<=count($halaman); $i++){
	if ($i==$hal+1)
		echo $i;
	else
		echo '<a href="'.$dir->rootDir().'arsip/'.$i.'/data.ayong" style="padding:0 5px;">'.$i.'</a>';
}
?></div>
</div>