...
function post($id, $dir, $query){
	...
	while ($i<count($_FILES["file"]["name"])){
		if ($_FILES["file"]["error"][$i] > 0){
			...
			$query->foto_album($nama.'.'.$ext, $data['name'], '',  $data['size'], $ext, $id);
			move_uploaded_file($data['tmp'], '../../../pcontent/upload/album/'.$nama.'.'.$ext);
			//$data['url']="openPage('".$nama.'.'.$ext."')";
			$data['thumbnail']=dirname($dir->rootDir())."/pcontent/upload/album/gambar.php?img=".$nama.'.'.$ext."&size=30&r=H";
